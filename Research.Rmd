...
We will extract only corresponding piece... from end of October to beginning November

```{r}
# extract approximate date and choose only relevant columns
bull_volat <- prices %>% filter(X1 > "2017-10-31", X1 < "2017-11-02") %>% select(X1, X23)
```

... and visualize it to confirm 

```{r}
ggplot(bull_volat, aes(X1, X23))+geom_line()
```

next, we can extract corresponding piece of macd dataframe:

```{r}
macd_bull_volat <- macd %>% select(X1, X10) %>% inner_join(bull_volat, by = c("X1" = "X1"))
```

and visualize both things together

```{r}
ggplot(macd_bull_volat, aes(X1, X10.y, col = X10.x))+geom_line()
```
